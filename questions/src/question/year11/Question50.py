...
class Question50(BaseQuestion):
    ...
    def graphic(self):
        ...
        shift_x_str = self.shift_x
        if self.shift_x > 0:
            shift_x_str = f'+{self.shift_x}'
        question = f'Given the graph of y=f(x), draw y=f(x {shift_x_str})'
    ...
    def answer_graphic(self):
        ...
        ax.plot([1.0-self.shift_x, 4.0-self.shift_x], [0.0, 3.0], 'r')
        ax.plot([1.0-self.shift_x, -5.0-self.shift_x], [0.0, 5.0 ], 'r')
        shift_y_str = self.shift_x
        if self.shift_x > 0:
            shift_y_str = f'+{self.shift_x}'
