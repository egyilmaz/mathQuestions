...
class Question53(BaseQuestion):
    ...
    def __init__(self):
        ...
        cv *= cv
        cv2 *= cv2 
        self.q = f"{a} is inversely proportional to {b} and {b} is directly proportional to square root of {c}. Given {a}={av}, {b}={bv} and {b}={bv2}, {c}={cv}. What is {a} when {c} is {cv2}"
        k = av*bv
        m = bv2/math.sqrt(cv)
        res = k/(m*math.sqrt(cv2))
        self.res= f"{a}={res}"
