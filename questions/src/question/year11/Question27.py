...
class Question27(BaseQuestion):
    def __init__(self):
        ...
        self.x = [1, 4, 2]
        self.y = [2, 4, 4]
        self.ang = random.choice([-90, -180, -270, 0, 90, 180, 270])
        self.r_x,self.r_y = random.choice([(0,0),(1,1),(2,2)])
        tri_ang = [(self.x[i],self.y[i]) for i in range(3)]
        self.body = f"Given a triangle with points on {tri_ang}, what must be the rotation angle and the rotation point, to draw the second triangle"

    def question(self):
        return self.body

    def answer(self):
        return f"rotation point is {(self.r_x, self.r_y)} rotation angle must be {self.ang}"
